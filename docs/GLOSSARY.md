...
- **Layer as schema** — the medallion layer is the Unity Catalog *schema* the table lives in (`parvum.gold.client_wealth`), not a prefix on its name (`gold_client_wealth`). The prefix form is a Hive-metastore holdover from the two-level `database.table` namespace; with three-level `catalog.schema.table` the middle level carries the layer, so grants, discovery and lineage attach to it. Here the whole estate is one `parvum` catalog with `bronze`/`silver`/`gold`/`dq`/`governance` schemas; the raw landing volume stays in `workspace.parvum.landing`.
...
- **`wait_timeout` / `PENDING` (SQL Statements API)** — a submit waits up to `wait_timeout` (50s, its maximum) for the statement to finish. If it hasn't, the API answers **HTTP 200** with state `PENDING` and a `statement_id` to poll — a success meaning *"still working, ask again"*, not a failure. A serverless warehouse that has auto-stopped must cold-start first (6m03s measured), which no `wait_timeout` can cover, so a client has to poll to completion (D-091).
- **Cold start (serverless warehouse)** — an auto-stopped warehouse spins a cluster up before it can run anything; queries queue in `PENDING` meanwhile. Cheap to leave stopped, slow on the first query after — which is why a reader's patience, not just its error handling, is part of its correctness.
...
- **Caddy** — a reverse proxy that obtains and renews TLS certificates automatically. Used here as a *shared* proxy: one site block per hostname, so several projects share port 443 on one host without each needing its own.
- **ACME / HTTP-01 challenge** — the protocol Let's Encrypt uses to prove you control a domain. HTTP-01 serves a token over **port 80**, which is why 80 must be open even when everything real is on 443, and why a proxying CDN in front of the origin breaks issuance.
- **Forced command (`command=` in `authorized_keys`)** — an SSH restriction that makes the server run one fixed program regardless of what the client asked for, putting the client's request in `SSH_ORIGINAL_COMMAND`. Turns a key that would grant a shell into one that can perform a closed list of actions (D-092).
- **`permitopen`** — an `authorized_keys` restriction limiting which host:port a key may tunnel to. Paired with `restrict` (deny everything, then re-enable named capabilities), it is how a CI key here can reach Postgres and nothing else; sshd refuses anything else with `administratively prohibited`.
- **SSH tunnel (local port forward)** — `-L local:host:port`, forwarding a port over an authenticated, encrypted SSH session. Lets CI reach a database bound to loopback on the far side, so the database needs no public listener at all.
- **TIMEOUT vs REFUSED** — the two ways a TCP connection fails, and a cheap diagnostic: a *timeout* means packets are being dropped by a filter upstream; a *refusal* means they arrived and nothing was listening. Distinguishes "firewall" from "service down" without guessing.
...
- **Cross-field invariant** — an assertion that two or more *published* figures agree about the same fact: an aggregate equals the parts that reconstruct it. Distinct from every other check in this estate, which validates a figure against its own source. It is the only shape that catches a defect where each number is individually correct and two of them disagree (D-072's MOIC, whose symptom — `called + unfunded ≠ commitment` — sat in a gold table for weeks). Writing one needs no theory of what might break it, which is the point.
- **Tautological control** — a check that cannot fail because the thing it asserts is how the value was computed. Worth keeping when it guards rounding or a future regression; never worth *citing* as coverage, because it inflates a governance number while testing nothing. D-073 walked back four such citations that would have read as 97% coverage instead of an honest 84.8%.
- **MOIC (multiple on invested capital)** — `(distributions + current NAV) / capital called` for a private fund. A *multiple*, not a rate: it is neither annualised nor time-weighted, it covers the fund's whole life rather than a chosen window, and its denominator is money actually put in rather than portfolio value. So it is not comparable to a portfolio TWR/Dietz/IRR over a reporting period, and a fund at 1.1× sitting inside a portfolio down 4% is not a contradiction.
- **One document, one moment** — the rule D-072 settled on for any ratio built from extracted documents: every term must come from the same source document and the same as-of date. Mixing a confirmed statement's NAV with a separately-confirmed set of call notices inflated MOIC fourfold and made it move with review progress instead of fund performance. Applies to any figure whose numerator and denominator can be confirmed independently.
- **Book restatement** — a change to how the book itself is constructed (here, an account’s `share_divisor`) that moves reported value without any market event or client cash flow. Same holdings, same prices, a different ruler. Distinct from the source-side restatement above, and invisible to a return chain that knows only wealth-yesterday vs wealth-today — which is how a divisor change once reported a +414% day (D-070).
- **Declared vs. inferred control** — the pairing D-070 settled on: the book *declares* a restatement (so a legitimate quarterly filing-regime step is never mistaken for one), and an independent check *infers* nothing but bounds the plausible, so an undeclared restatement still surfaces. Either half alone is unsound — declaration without detection is a licence to explain away any number; detection without declaration cannot tell a real lumpy return from an artefact.
- **Plausibility band** — a stated bound on how far a figure may move before it needs an explanation (25% day-over-day, net of flows, in `dq.return_plausibility`). Deliberately a judgement rather than a value fitted to observed history: a bound tuned to what has already happened is a bound that will not catch what has not.
...
- **SLO attainment** — the share of measured days on which the metric evidencing a service level actually passed, over a trailing window. What turns a stated objective into one that can be missed rather than only quoted.
- **Error budget** — how much of an SLO you are allowed to miss before it is breached: `(1 − attainment_objective) × days measured`. An objective of 1.0 has *no* budget by construction, which is why a correctness invariant is a poor fit for the SLO shape and is reported as "none" rather than as 0% or 100%.
- **Handoff standard** — the written contract that lets a team who did not build a system operate it: per alert, what it means, the first checks, the action, and the escalation point. `docs/RUNBOOK.md`. The artefact that makes "ops teams can own production feeds" true rather than aspirational.
- **Column contract** — machine-checkable metadata about how a table is used rather than what it contains: its grain, its foreign keys and their cardinality, and what it is for. Conventionally written into catalog comments where nothing verifies them; here the gate resolves both ends of every declared join against the published inventory, so a contract that stops being true fails the build.
- **Join cardinality** — whether a declared join can fan out (`many_to_one`) or is one row to one row (`one_to_one`). The property that decides whether a join silently multiplies a sum, which is why it is a closed set rather than free text.
- **Grain (declared)** — the columns that make one row of a table unique, stated in the register rather than inferred. `silver.positions` and `silver.position_owners` have nearly the same columns and completely different grains; mixing them is how a shared account gets double-counted.
