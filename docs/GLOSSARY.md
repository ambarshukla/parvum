...
- **Grain (declared)** — the columns that make one row of a table unique, stated in the register rather than inferred. `silver_positions` and `silver_position_owners` have nearly the same columns and completely different grains; mixing them is how a shared account gets double-counted.
