...
- **DQ dimensions (freshness / completeness / accuracy)** — the standard vocabulary for classifying a data-quality check by *what kind of question it answers*: freshness asks whether data is recent enough; completeness asks whether everything expected arrived; accuracy asks whether what arrived is correct. A metrics table organized by dimension lets a reader ask "which kind of problem are we having" before drilling into which specific check is failing.
- **Declarative metrics rollup** — a table shaped so that adding a new check costs one more row-producing query, never a schema migration: long/narrow (one row per date × dimension × metric) rather than wide (one column per metric). The tradeoff for that flexibility is that every consumer has to filter by `metric` rather than just naming a column.
...
- **Data governance** — the function that decides who is accountable for which data, what it is allowed to mean, and how that accountability is evidenced. Distinct from data quality: quality asks whether a number is right, governance asks who owes an answer when it isn't.
- **Critical Data Element (CDE)** — a specific, named field whose being wrong causes material harm — a client statement that misstates wealth, a return figure a decision is taken on. Deliberately a *short* list: the discipline only works if most fields are not critical, because "everything is critical" carries no information.
- **Tier (criticality tier)** — the classification a column carries in the register. Here: `critical` (the business consumes it directly), `supporting` (feeds or qualifies a critical element), `operational` (pipeline plumbing — timestamps, hashes, provenance).
- **Data owner** — the role accountable for an element's definition and quality. A *role*, not a person: ownership by role survives people changing jobs, which is the property that makes the register durable.
- **Publisher obligations** — what an owner owes for an element they publish: a business definition, a service level, and a control that tests it. "Automating publisher responsibilities into workflows" means the platform refuses work that skips them, rather than a governance team chasing people for spreadsheet updates.
- **Governance gate** — a CI check that fails a change which leaves the register and the published schema out of step. The mechanism that turns a register from a document into a control.
- **Control gap** — an explicit, recorded admission that a critical element has no automated check yet, naming what is missing and what would close it. Preferred to silence *and* to a loosely-related rule cited to look covered: an unmeasured element you know about is manageable, one you have papered over is not.
- **Governance coverage** — the share of published columns classified in the register, and of critical elements that have a real control rather than a stated gap. The pair of numbers a governance function reports upward.
- **Business definition vs. catalog description** — the catalog comment says what a column technically contains; the business definition says what it means and why being wrong matters. The register carries the second for critical elements, and checks the first exists for every column.
- **AI-consumable / AI-ready data** — data a model can use without a human explaining it first: it has lineage (where it came from), schema (what shape it is), quality signals (whether to trust it), and an ontology (what the terms mean). The same four properties a new analyst needs, which is why governance work and AI-readiness work are largely the same work.
