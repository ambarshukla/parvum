-- performance_metrics — governed measures over gold.performance.
--
...
-- chained figures stay in `gold.performance_summary`, computed once, by the
...
CREATE OR REPLACE VIEW parvum.gold.performance_metrics
WITH METRICS
LANGUAGE YAML
AS $$
version: 0.1
source: parvum.gold.performance
dimensions:
  - name: Client
    expr: client_name
  - name: As of
...
COMMENT ON VIEW parvum.gold.performance_metrics IS
...
COMMENT ON COLUMN parvum.gold.performance_metrics.`Wealth` IS
...
COMMENT ON COLUMN parvum.gold.performance_metrics.`Net external flow` IS
...
COMMENT ON COLUMN parvum.gold.performance_metrics.`Restatement adjustment` IS
...
COMMENT ON COLUMN parvum.gold.performance_metrics.`Days` IS
  'Number of client-days in the selected slice';
COMMENT ON COLUMN parvum.gold.performance_metrics.`Client` IS
  'Client family display name';
COMMENT ON COLUMN parvum.gold.performance_metrics.`As of` IS
