-- allocation_metrics — governed measures over gold.asset_allocation.
--
...
CREATE OR REPLACE VIEW parvum.gold.allocation_metrics
WITH METRICS
LANGUAGE YAML
AS $$
version: 0.1
source: parvum.gold.asset_allocation
dimensions:
  - name: Client
    expr: client_name
  - name: As of
...
COMMENT ON VIEW parvum.gold.allocation_metrics IS
  'Governed allocation measures: what client wealth is made of, by asset class and date. Semantic layer over gold.asset_allocation — see docs/SEMANTIC_LAYER.md.';

COMMENT ON COLUMN parvum.gold.allocation_metrics.`Allocated value` IS
...
COMMENT ON COLUMN parvum.gold.allocation_metrics.`Asset classes` IS
  'How many distinct asset classes appear in the selected slice';
COMMENT ON COLUMN parvum.gold.allocation_metrics.`Clients` IS
  'Distinct client families in the selected slice';
COMMENT ON COLUMN parvum.gold.allocation_metrics.`Client` IS
  'Client family display name';
COMMENT ON COLUMN parvum.gold.allocation_metrics.`As of` IS
  'Valuation date; the grain of the source is one row per client per date per asset class';
COMMENT ON COLUMN parvum.gold.allocation_metrics.`Asset class` IS
